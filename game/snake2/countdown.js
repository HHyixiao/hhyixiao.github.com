window.onload=function(){
	...
	var update = function (modifier) {
		...
		// 限制多余坐标
		if (Math.abs(Math.floor(snake.x) - snake.body[0].x) >snake.r || Math.abs(Math.floor(snake.y) - snake.body[0].y) >snake.r) {
	...
	var render = function () {
		...
		for (var i in snake.body) {
			context.beginPath()
			context.arc(snake.body[i].x - ditu.item.x, snake.body[i].y - ditu.item.y, snake.r , 0 , 2*Math.PI)
			context.fill()
